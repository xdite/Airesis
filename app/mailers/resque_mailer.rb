...
class ResqueMailer < ActionMailer::Base
  ...
  def admin_message(msg)
    @msg = msg
    mail(:to => 'coorasse+daily@gmail.com', :subject => Setting.app_short_name + " - Messaggio di amministrazione")
  end

  def report_message(report_id)
    @report = ProposalCommentReport.find(report_id)

    mail(:to => 'coorasse+report@gmail.com', :subject => Setting.app_short_name + " - Segnalazione Contributo")
  end


  def info_message(msg)
    mail(:to => 'coorasse+info@gmail.com', :subject => Setting.app_short_name + " - Messaggio di informazione")
  end
